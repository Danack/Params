...
function createArrayOfTypeFromInputStorage(
    ...
    $items = [];
    /** @var \Type\ValidationProblem[] $allValidationProblems */
...
{
    return getPropertyDefinitionsForClass($className);
}
 ...
 * @throws TypeNotInputParameterListException
 */
function getPropertyDefinitionsForClass(string $className): array
{
    ...
    $inputParameterList = getPropertyDefinitionsFromAnnotations($className);
    ...
    if (count($inputParameterList) === 0) {
        ...
        $inputParameterList = call_user_func([$className, 'getPropertyRulesList']);
    }
    ...
    foreach ($inputParameterList as $inputParameter) {
        if (!$inputParameter instanceof PropertyDefinition) {
    ...
    /** @var \Type\PropertyDefinition[] $inputParameterList */
 ...
 * @template T
 * @param class-string<T> $classname
 * @param \Type\PropertyDefinition[] $params
 * @param DataStorage $dataStorage
 * @return T of object
 * @throws ValidationException
 * @throws \ReflectionException
...
/**
 * @param \Type\PropertyDefinition $param
...
/**
 * @param \Type\PropertyDefinition[] $inputParameters
 * @param ProcessedValues $paramValues
 * @param DataStorage $dataStorage
 ...
 * @throws \ReflectionException
 */
function getPropertyDefinitionsFromAnnotations(string $class): array
{
...
function validate(object $dto)
{
    ...
    $inputParameterList = getPropertyDefinitionsForClass($class);
